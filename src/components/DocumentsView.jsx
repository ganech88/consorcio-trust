...
const STATUS_CONFIG = {
  pending:      { label: 'Pendiente',   badge: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' },
  approved:     { label: 'Aprobado',    badge: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' },
  rejected:     { label: 'Rechazado',   badge: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300' },
  under_review: { label: 'En revisión', badge: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' },
};
...
export default function DocumentsView({ session, userProfile }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-between gap-4">
        <div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-sm shrink-0"
        >
          <Plus size={16} />
          Subir documento
        </button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700">
              <h4 className="font-bold text-slate-800 dark:text-slate-100">Subir documento</h4>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={() => { setShowForm(false); setForm(EMPTY_FORM); }}
                  className="flex-1 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                >
                  Cancelar
                </button>
                <button
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-16 text-center">
          ...
          <h4 className="font-semibold text-slate-600 dark:text-slate-300">No tenés documentos subidos</h4>
        ...
        <div className="space-y-3">
          {docs.map(doc => {
            ...
            return (
              <div
                key={doc.id}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-4"
              >
                <div className="flex items-start gap-3">
                  <div className="bg-brand-50 dark:bg-brand-900/30 p-2.5 rounded-xl shrink-0">
                    <FileText size={18} className="text-brand-600 dark:text-brand-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{doc.title}</p>
                    ...
                    {doc.admin_notes && (
                      <div className="mt-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl p-3">
                        <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-0.5">
                          Nota del administrador
                        </p>
                        <p className="text-sm text-slate-700 dark:text-slate-300">{doc.admin_notes}</p>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    {doc.file_url && (
                      <a
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-slate-700 transition-colors"
                        title="Ver archivo"
                      >
                        <FileText size={15} />
                      </a>
