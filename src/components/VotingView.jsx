...
function ProgressBar({ percent, color }) {
  return (
    <div className="h-2 bg-slate-100 dark:bg-surface-panel2 rounded-full overflow-hidden">
      <div
  ...
  return (
    <div className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] overflow-hidden">
      ...
      <div className="p-5 pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                active
                  ...
                  : 'bg-slate-100 text-slate-500 dark:bg-surface-panel2 dark:text-ink-mid'
              }`}>
            ...
            <h4 className="font-bold text-slate-800 dark:text-ink-hi">{poll.title}</h4>
            {poll.description && (
              <p className="text-sm text-slate-500 dark:text-ink-mid mt-1">{poll.description}</p>
            )}
          </div>
          {isAdmin && (
            <button
      ...
      <div className="px-5 pb-5 space-y-2">
              ...
              <div key={idx} className="space-y-1.5">
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                    isMyVote
                      ? 'border-blue-400 bg-blue-50 dark:bg-brand-400/[0.14] text-blue-700 dark:text-brand-400'
                      : hasVoted || !active
                      ? 'border-slate-200 dark:border-white/[0.07] bg-slate-50 dark:bg-surface-inset text-slate-600 dark:text-ink-mid cursor-default'
                      : 'border-slate-200 dark:border-white/[0.07] hover:border-blue-300 dark:hover:border-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-slate-700 dark:text-ink-hi cursor-pointer'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    {showResults && (
                      <span className="text-xs font-bold text-slate-500 dark:text-ink-mid shrink-0 ml-2">
                        {count} ({percent}%)
                      </span>
                    )}
                  </div>
        ...
        <p className="text-xs text-slate-400 dark:text-ink-low text-right pt-1">
...
function CreatePollForm({ session, userProfile, onCreated }) {
  ...
  return (
    ...
    >
      <h4 className="font-bold text-slate-800 dark:text-ink-hi text-sm">Crear nueva encuesta</h4>
      ...
      <div>
        <label className="text-xs font-semibold text-slate-500 dark:text-ink-mid mb-1.5 block">Fecha de cierre</label>
        <input
          type="datetime-local"
          value={form.endsAt}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-semibold text-slate-500 dark:text-ink-mid">Opciones</label>
          <button
            type="button"
            onClick={addOption}
            disabled={form.options.length >= 8}
            className="text-xs text-blue-600 dark:text-brand-400 hover:underline disabled:opacity-40 flex items-center gap-1"
          >
          ...
          {form.options.map((opt, idx) => (
            <div key={idx} className="flex gap-2">
              <input
                ...
                className="flex-1 border border-slate-200 dark:border-white/[0.09] rounded-xl px-3 py-2 text-sm bg-white dark:bg-surface-panel2 dark:text-ink-hi focus:ring-2 focus:ring-blue-500 outline-none"
              />
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {polls.length === 0 && (
        <div className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] p-12 text-center">
          <Vote size={40} className="mx-auto text-slate-300 dark:text-ink-low mb-3" />
          <p className="text-slate-500 dark:text-ink-mid text-sm font-medium">No hay votaciones activas</p>
          {isAdmin && (
            <button
