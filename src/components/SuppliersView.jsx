...
const ORDER_STATUS = {
  ...
  approved:  { label: 'Aprobada',  color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-brand-400' },
  paid:      { label: 'Pagada',    color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400' },
  cancelled: { label: 'Cancelada', color: 'bg-slate-100 text-slate-500 dark:bg-surface-panel2 dark:text-ink-mid' },
};
  ...
  async function handleCreateOrder(e) {
    ...
    try {
      const o = await createPaymentOrder(userProfile.consortium_id, session.user.id, {
        ...
        attachment_url: orderForm.attachment_url || null,
      });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-white dark:bg-surface-panel rounded-xl border border-slate-100 dark:border-white/[0.07] p-4">
          <p className="text-xs text-slate-400 dark:text-ink-low mb-1">Pendiente de pago</p>
          <p className="text-xl font-bold text-amber-600 dark:text-amber-400">{formatCurrency(pendingTotal)}</p>
        </div>
        <div className="bg-white dark:bg-surface-panel rounded-xl border border-slate-100 dark:border-white/[0.07] p-4">
          <p className="text-xs text-slate-400 dark:text-ink-low mb-1">Pagado este mes</p>
          <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(paidThisMonth)}</p>
        </div>
        <div className="bg-white dark:bg-surface-panel rounded-xl border border-slate-100 dark:border-white/[0.07] p-4">
          <p className="text-xs text-slate-400 dark:text-ink-low mb-1">Proveedores activos</p>
          <p className="text-xl font-bold text-slate-800 dark:text-ink-hi">{suppliers.length}</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-slate-100 dark:bg-surface-panel p-1 rounded-xl">
        ...
        <div className="space-y-4">
            ...
            <form onSubmit={handleCreateSupplier} className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={f.key}>
                    <label className="text-xs font-semibold text-slate-500 dark:text-ink-mid mb-1.5 block">{f.label}</label>
                    <input
                      type="text"
          ...
          {suppliers.length === 0 ? (
            <div className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] p-12 text-center">
              <Truck size={40} className="mx-auto text-slate-300 dark:text-ink-low mb-3" />
              <p className="text-slate-500 dark:text-ink-mid text-sm">No hay proveedores registrados</p>
            </div>
          ) : (
            <div className="space-y-2">
              {suppliers.map(s => {
                ...
                return (
                  <div key={s.id} className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] overflow-hidden">
                    <div className="p-4 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center shrink-0">
                        <Building size={18} className="text-blue-500 dark:text-brand-400" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-800 dark:text-ink-hi text-sm">{s.name}</p>
                        <div className="flex items-center gap-2 flex-wrap mt-0.5">
                          <span className="text-[10px] font-bold bg-blue-100 dark:bg-brand-400/[0.14] text-blue-700 dark:text-brand-400 px-1.5 py-0.5 rounded">{s.category}</span>
                          {s.cuit && <span className="text-xs text-slate-400 dark:text-ink-low">CUIT: {s.cuit}</span>}
                          {s.phone && <span className="flex items-center gap-1 text-xs text-slate-400 dark:text-ink-low"><Phone size={10} />{s.phone}</span>}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        {supplierOrders.length > 0 && (
                          <span className="text-xs font-bold bg-slate-100 dark:bg-surface-panel2 text-slate-500 dark:text-ink-mid px-2 py-0.5 rounded-full">
                    ...
                    {isExpanded && (
                      <div className="border-t border-slate-100 dark:border-white/[0.07] px-4 py-3 bg-slate-50 dark:bg-white/[0.04] space-y-1.5">
                        {s.email && <p className="text-xs text-slate-500 dark:text-ink-mid flex items-center gap-2"><Mail size={12} />{s.email}</p>}
                        {s.bank_info && <p className="text-xs text-slate-500 dark:text-ink-mid flex items-center gap-2"><CreditCard size={12} />{s.bank_info}</p>}
                        {s.notes && <p className="text-xs text-slate-400 dark:text-ink-low italic">{s.notes}</p>}
                        {supplierOrders.length === 0 && <p className="text-xs text-slate-400 dark:text-ink-low">Sin órdenes de pago</p>}
                      </div>
                    )}
                  </div>
                );
      ...
      {activeTab === 'ordenes' && (
        <div className="space-y-4">
          ...
          {showOrderForm && (
            <form onSubmit={handleCreateOrder} className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] p-5 space-y-4">
              <h4 className="font-bold text-slate-800 dark:text-ink-hi flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500 dark:text-ink-mid mb-1.5 block">N° Factura</label>
                  <input
                    type="text"
                ...
                <div className="sm:col-span-2">
                  <label className="text-xs font-semibold text-slate-500 dark:text-ink-mid mb-1.5 block">
                    Factura o comprobante (imagen/PDF)
                  </label>
                  <FileUpload
                    value={orderForm.attachment_url}
                    onChange={url => setOrderForm(prev => ({ ...prev, attachment_url: url || '' }))}
                    folder="payment-orders"
                    label="Adjuntar factura"
                  />
                </div>
              </div>
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowOrderForm(false)} className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-500 hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors">Cancelar</button>
          ...
          {orders.length === 0 ? (
            <div className="bg-white dark:bg-surface-panel rounded-2xl border border-slate-100 dark:border-white/[0.07] p-12 text-center">
              <CreditCard size={40} className="mx-auto text-slate-300 dark:text-ink-low mb-3" />
              <p className="text-slate-500 dark:text-ink-mid text-sm">No hay órdenes de pago registradas</p>
            </div>
          ) : (
            <div className="space-y-2">
              {orders.map(o => {
                ...
                return (
                  <div key={o.id} className={`bg-white dark:bg-surface-panel rounded-2xl border p-4 flex items-start gap-3 ${isOverdue ? 'border-red-200 dark:border-red-800' : 'border-slate-100 dark:border-white/[0.07]'}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold text-slate-800 dark:text-ink-hi text-sm">{o.suppliers?.name}</p>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${st.color}`}>{st.label}</span>
                        {isOverdue && <span className="text-[10px] font-bold text-red-500 dark:text-red-400">VENCIDA</span>}
                      </div>
                      <p className="text-xs text-slate-500 dark:text-ink-mid mt-0.5">{o.description}</p>
                      <div className="flex items-center gap-2 mt-1 flex-wrap text-xs text-slate-400 dark:text-ink-low">
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <p className="font-bold text-slate-800 dark:text-ink-hi">{formatCurrency(o.amount)}</p>
                      <div className="flex items-center gap-1">
                        ...
                        {o.status === 'pending' && (
                          <button
                            ...
                            title="Cancelar"
                            className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors"
                          >
                            <X size={15} />
                          </button>
                        )}
